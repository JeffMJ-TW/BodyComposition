...
app.post('/api/analyze-health', async (req, res) => {
  try {
    ...
    if (req.body.useFallback) {
      console.log('Immediate clinical fallback requested');
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn('GEMINI_API_KEY is not set, auto-generating clinical analysis fallback');
      const fallbackResult = generateFallbackClinicalAnalysis(records);
      return res.json({
        success: true,
        data: fallbackResult,
    ...
    // Summary of latest record and comparison
...
1. 觀察「體重」、「體脂肪率(%)」、「體脂肪量(kg)」、「骨骼肌率(%)」與「骨骼肌重量(kg)」的互動關係。
   - 警惕「虛假減重」：如果體重下降，但骨骼肌重量下降、體脂肪率反而上升（如近期數據顯示），必須溫和但明確提醒「可能減到了水分與肌肉，而非真實脂肪，需要注意蛋白質攝取與阻力訓練避免肌少症或代謝下降」。
...
請以繁體中文回答，語氣專業、同理心且具鼓勵性。`;
    ...
    const CANDIDATE_MODELS = ['gemini-3.1-flash-lite', 'gemini-3.8-flash', 'gemini-3.6-flash'];
    const PER_MODEL_TIMEOUT_MS = 6500; // 6.5s timeout per candidate to guarantee fast user experience
    ...
    for (const modelName of CANDIDATE_MODELS) {
      try {
        console.log(`Attempting AI health analysis with model: ${modelName} (timeout: ${PER_MODEL_TIMEOUT_MS}ms)`);
          ...
          config: {
            ...
            responseSchema: {
              ...
              properties: {
                overallAssessment: {
                  type: Type.STRING,
                  description: '整體健康狀況綜合簡評（約 80-120 字）',
                },
                weightLossQuality: {
                  type: Type.OBJECT,
                  properties: {
                    level: {
                      type: Type.STRING,
                      description: '減重品質分類：優秀減脂 / 肌肉流失警訊 / 體態維持 / 脂肪上升',
                    },
                    summary: {
                      type: Type.STRING,
                      description: '針對肌肉與脂肪消長關係的精準分析說明',
                    },
                  },
                ...
                keyObservations: {
                  ...
                  description: '3-4 項關鍵數值觀察指標（包含內臟脂肪、骨骼肌率、代謝年齡等）',
                },
                segmentalAnalysis: {
                  type: Type.STRING,
                  description: '四肢與身軀肌肉及皮下脂肪分布與平衡點評',
                },
                actionPlan: {
                  type: Type.OBJECT,
                  properties: {
                    diet: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: '飲食調整具體建議（至少 2-3 點）',
                    },
                    exercise: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: '運動訓練建議（重訓 vs 有氧建議）',
                    },
                    lifestyle: {
                      type: Type.ARRAY,
                      items: { type: Type.STRING },
                      description: '生活作息與測量注意事項（如測量時間、水分維持）',
                    },
                  },
        ...
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error(`Model ${modelName} call exceeded ${PER_MODEL_TIMEOUT_MS}ms`)), PER_MODEL_TIMEOUT_MS)
        );
        ...
        if (text) {
          ...
          usedModel = modelName;
          console.log(`Successfully generated AI analysis with model: ${modelName}`);
          break; // Success!
        }
      } catch (err: any) {
        console.warn(`Model ${modelName} call failed or timed out, trying next candidate:`, err?.message || err);
        lastError = err;
      }
    }

    // If all models failed or encountered 503 spikes, use clinical fallback to prevent breaking UI
    if (!parsedResult) {
      console.warn('All Gemini candidate models failed or busy. Falling back to clinical algorithmic analysis.');
    ...
    let friendlyMessage = '產生 AI 分析時發生錯誤，請稍後再試。';
    if (error?.message) {
      try {
        const parsed = JSON.parse(error.message);
        friendlyMessage = parsed?.error?.message || parsed?.message || error.message;
      } catch {
